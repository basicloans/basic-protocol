...
contract BToken is CToken {
    ...
    function notifySavingsChange(address addr) internal {
        BasicConfig basicConfig = Basictroller(address(comptroller)).basicConfig();
        ILiquidityGauge liquidityGauge = basicConfig.liquidityGauge();
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        (vars.liquidatorSeizeTokens, vars.safetyVaultTokens) = basicConfig.calculateSeizeTokenAllocation(seizeTokens, liquidationIncentive);
        address safetyVault = basicConfig.safetyVault();
        /*
    ...
    function doBorrowTransferOut(address payable to, uint amount) internal {
        BasicConfig basicConfig = Basictroller(address(comptroller)).basicConfig();
        address payable safetyVault = address(uint160(basicConfig.safetyVault()));
        uint borrowFeeRatio = basicConfig.marketBorrowFeeRatio(address(this));
        uint borrowFee = mul_(amount, borrowFeeRatio)/mantissaOne;
        uint noFeeAmount = sub_(amount, borrowFee);
        doTransferOut(to, noFeeAmount);
        doTransferOut(safetyVault, borrowFee);
    }
    /**
    ...
    function maxFlashLoan(address token) external view returns (uint256) {
        ...
        return Basictroller(address(comptroller)).getFlashLoanCap(address(this));
    }
    ...
    function getFlashFeeInternal(address token, uint256 amount) internal view returns (uint256) {
        token;
        return Basictroller(address(comptroller)).basicConfig().getFlashFee(msg.sender, address(this), amount);
    }
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 amount, bytes calldata data) external returns (bool) {
        ...
        Basictroller(address(comptroller)).flashLoanAllowed(address(this), address(receiver), amount);
