...
contract BasicConfig is Ownable, Exponential {
    ...
    mapping(address => uint) public marketBorrowFeeRatio;
    ...
    event NewBorrowFeeRatio(address cToken, uint oldValue, uint newValue);
    ...
    constructor(BasicConfig previousBasicConfig) public {
        safetyGuardian = msg.sender;
        if (address(previousBasicConfig) == address(0x0)) return;
       
        safetyGuardian = previousBasicConfig.safetyGuardian();
        compToken = previousBasicConfig.compToken();
        safetyVaultRatio = previousBasicConfig.safetyVaultRatio();
        safetyVault = previousBasicConfig.safetyVault();
    }
    ...
    function _setMarketBorrowFeeRatio(address _market, uint _borrowFeeRatio) external onlySafetyGuardian {
        require(_borrowFeeRatio <= 0.5e18, "Invalid borrow fee ratio");
        
        emit NewBorrowFeeRatio(_market, marketBorrowFeeRatio[_market], _borrowFeeRatio);

        marketBorrowFeeRatio[_market] = _borrowFeeRatio;
    }
