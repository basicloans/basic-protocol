...
import "./BasicConfig.sol";

contract Basictroller is Comptroller {

    BasicConfig public basicConfig;

    function _setBasicConfig(BasicConfig _basicConfig) public {
    ...
    function calculateSeizeTokenAllocation(uint _seizeTokenAmount) public view returns(uint liquidatorAmount, uint safetyVaultAmount) {
        return basicConfig.calculateSeizeTokenAllocation(_seizeTokenAmount, liquidationIncentiveMantissa);
    }
    ...
    function transferComp(address user, uint userAccrued, uint threshold) internal returns (uint) {
        ...
        if (userAccrued >= threshold && userAccrued > 0 && compAddress != address(0x0)) {
            ...
            if (userAccrued <= compRemaining) {
                (uint userAmount, uint governanceAmount) = basicConfig.getCompAllocation(user, userAccrued);
                if (userAmount > 0) comp.transfer(user, userAmount);
                if (governanceAmount > 0) comp.transfer(basicConfig.safetyVault(), governanceAmount);
                return 0;
            }
        }
        return userAccrued;
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        uint borrowCap = basicConfig.getBorrowCap(cToken);
    ...
    function flashLoanAllowed(address cToken, address to, uint256 flashLoanAmount) view public returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!borrowGuardianPaused[cToken], "paused");
        require(basicConfig.whitelist(to), "!whitelist");

        if (!markets[cToken].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }

        uint flashLoanCap = basicConfig.getFlashLoanCap(cToken);
    ...
    function getFlashLoanCap(address cToken) view external returns (uint) {
        return basicConfig.getFlashLoanCap(cToken);
    }
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external returns (uint) {
        require(!basicConfig.isBlocked(minter));
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        uint supplyCap = basicConfig.getSupplyCap(cToken);
  ...
  */
    ...
    */
    function getHypotheticalAccountLiquidityInternal(
        ...
        if (basicConfig.getCreditLimit(account) == uint(-1)) {
        ...
        if (basicConfig.getCreditLimit(account) > 0) {
            vars.sumCollateral = basicConfig.getCreditLimit(account);
        }
    ...
    function liquidateBorrowAllowed(
        ...
        require(basicConfig.getCreditLimit(borrower) == 0 , "credit");
    ...
    function repayBorrowAllowed(
        ...
        address borrower,
        uint repayAmount) public returns (uint) {
        require(basicConfig.getCreditLimit(borrower) == 0 || payer == borrower, "Payer != borrower");
    ...
    function safetyGuardian() external view returns (address) {
        return basicConfig.safetyGuardian();
    }
